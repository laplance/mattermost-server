...
func (h Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if h.IsStatic {
		...
		w.Header().Set("X-Frame-Options", "allow-from http://wooj.yht.co.kr:8080 http://10.3.1.15:8080 http://wooj.yht.co.kr")		
		// Set content security policy. This is also specified in the root.html of the webapp in a meta tag.
		w.Header().Set("Content-Security-Policy", fmt.Sprintf(
			//"frame-ancestors 'self'; script-src 'self' cdn.segment.com/analytics.js/%s",
			"frame-ancestors 'self' allow-from http://wooj.yht.co.kr:8080 http://10.3.1.15:8080 http://wooj.yht.co.kr; script-src 'self' cdn.segment.com/analytics.js/%s",						
			h.cspShaDirective,
		))
	} else {
